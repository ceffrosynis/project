...
class Order(models.Model):
    ...
    def get_price(self):
        if self.ProductID.Discount == 0:
            return round(self.ProductID.Price, 2)
        else:
            return round(self.ProductID.Price - self.ProductID.Discount, 2)

    def total_price(self):
        if self.ProductID.Discount is None:
            return round(self.ProductID.Price * self.Quantity, 2)
        else:
            return round((self.ProductID.Price - self.ProductID.Discount) * self.Quantity, 2)
